...
export const DefaultDashboard = () => {
  ...
  const [congTyData, setCongTyData] = useState([])
  const { direction } = useSelector(state => state.theme)

  console.log(congTyData)
  useEffect(() => {
    const fetchData = async () => {
      const congTyDataFetched = await congTyService.getCongTy()
      setCongTyData(congTyDataFetched);
    }
    fetchData()
    return () => {
            ...
            <div className="mt-3">
              {
                ...
                //   </div>
                // ))
                congTyData?.congTy?.data?.slice(0, 5).map((elm, i) => (
                  <div key={i} className={`d-flex align-items-center justify-content-between mb-4`}>
                    <AvatarStatus id={i} src={`/img/images/banner/${elm?.logo?.replace(" ", "")?.toLowerCase()}` || "/img/avatars/thumb-2.jpg"} name={elm?.tenCongTy} subTitle={"abc"} />
                    <div>
                      <Button icon={<UserAddOutlined />} type="default" size="small">Add</Button>
                    </div>
                  </div>
